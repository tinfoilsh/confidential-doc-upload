...
| **Allowlisted router egress** | The secret-bearing router may connect only to `atc.tinfoil.sh` for client-verified attestation bundles and the ATC-advertised inference routers for end-to-end encrypted VLM requests. cvmimage enforces the resolved-IP allowlist outside the container. |
